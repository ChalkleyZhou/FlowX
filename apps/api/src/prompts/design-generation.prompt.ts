...
export const designGenerationPrompt: PromptTemplate = {
  ...
  user:
    '基于以下确认的产品简报，生成 UI 设计规格和 Demo 页面代码。输出 overview、pages（含 layout 线框描述）、demoScenario、dataModels、apiEndpoints、designRationale，以及 demoPages（可编译的页面组件代码，使用目标仓库的真实组件 import 路径和 API）。',
};
