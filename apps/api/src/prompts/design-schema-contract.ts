...
export function getDesignSpecSchemaContractBlock(): string {
  ...
  return [
    ...
    '你只输出一个能通过该校验的 JSON 对象；顶层含 design、demo、surfaces。surfaces 为按端多页 HTML（推荐 Web端/移动端/管理后台）。禁止 Markdown、禁止代码围栏。本阶段不要求 demoPages。',
    '',
    body,
  ].join('\n');
}
...
export function getDesignSpecSchemaSummaryContractBlock(): string {
  return [
    ...
    '只输出一个 JSON 对象；顶层键必须为 design、demo、surfaces（缺一不可）。禁止 Markdown、禁止用 ``` 围栏包裹。',
    '严禁用「...」「省略」「同上」代替真实内容；surfaces[].pages[].html 必须是完整 HTML 字符串（换行转义），可直接在 sandbox iframe 渲染。本阶段不要求 demoPages。',
    '',
    'design: { overview, pages: [≥1 项 { name, route, layout, keyComponents[], interactions[] }], demoScenario, designRationale }',
    'demo: { summary, flows: [{ name, goal, entry, states[] }], scope: { included[], excluded[] }, knownGaps[] }',
    'surfaces: [≥1 项 { id, pages: [{ id, title?, html }] }]；推荐 id 为 Web端 / 移动端 / 管理后台',
  ].join('\n');
}
