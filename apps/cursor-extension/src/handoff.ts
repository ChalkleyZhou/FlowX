...
export function buildPromptFromLocalHandoff(task: FlowXTaskItem, handoff: LocalHandoffPayload): string {
  ...
  return [
    ...
    '',
    '## Spec & Plan',
    `- Spec goal: ${handoff.specPlan.spec.goal.trim()}`,
    `- Spec scope: ${handoff.specPlan.spec.scope.join(', ')}`,
    `- Spec acceptance criteria: ${handoff.specPlan.spec.acceptanceCriteria.join('; ')}`,
    '',
    '### Plan approach',
    handoff.specPlan.plan.approach.trim(),
    '',
    `- Plan sequence: ${handoff.specPlan.plan.sequence.join(' -> ')}`,
    `- Plan touchpoints: ${handoff.specPlan.plan.touchpoints.join(', ')}`,
    `- Plan risks: ${handoff.specPlan.plan.risks.join(', ')}`,
    `- Plan verification: ${handoff.specPlan.plan.verification.join(', ')}`,
    '',
    '## Suggested checks',
