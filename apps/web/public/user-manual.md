...
如果当前工作流需要设计师在本地 OpenDesign 完成 UI/交互设计：

1. 先安装并启动本机 Agent：`npm install -g @flowx-ai/local`，然后运行 `flowx-local serve`（详见 [本地 Agent](/local-agent)）。
2. 在工作流详情进入 `设计方案` 阶段，等待状态变为 `待设计方案`。
3. 点击该阶段的 `打开本地 OpenDesign`，再在 Cursor Agent 中启用 `flowx-local mcp`，使用 `flowx_get_active_design_session` 和 `flowx_get_design_handoff` 获取当前工作流上下文。
4. 在 OpenDesign 中完成设计，通过 `flowx_submit_design` 提交结果；也可以更新会话目录中的 `result.json` 后，在工作流详情点击 `回传本地设计`。
5. 设计稿回传后，工作流进入 `待确认设计方案`，由人工确认、驳回或跳过后继续下一阶段。
...
2. 产品构思：生成或补充产品简报，可跳过
3. 设计方案：生成或回传设计稿，可确认、驳回或跳过
