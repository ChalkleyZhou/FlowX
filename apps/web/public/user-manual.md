...
云效自动化规则还可以调用固定地址 `/api/yunxiao-webhooks`，将原生“工作项数据”定向发送给负责人。云效配置的 Secret 会通过 `X-Projex-Signature` 请求头传递，FlowX 使用 `YUNXIAO_WEBHOOK_SECRET` 校验，不需要也不应向云效提供个人 API Token。接入前应先完成钉钉用户同步；负责人账号或姓名必须在钉钉组织成员中唯一匹配。组织管理员可以在“设置”→“云效集成”中一键启用或停用通知，停用不会删除配置和历史记录。完整配置见仓库文档 `docs/yunxiao-webhook.md`。
...
已安装 [本地 Agent](/local-agent) 时，也可在 Cursor / Codex 说明要新建 FlowX 需求。需求页次要入口「用 Cursor / Codex 发起」会打开该指南。Agent 会按 `flowx-intake-requirement` Skill：列出项目 → **确认发布版本（用当前 / 新建）** → 创建需求 → **经你确认后**启动工作流 → 可选进入产品构思。
