...
**推荐主路径：** 安装 [本地 Agent](/local-agent)，执行 `flowx-local setup` 与 `flowx-local login` 后，在 Cursor / Codex 说明要新建 FlowX 需求。Agent 会按 `flowx-intake-requirement` Skill：列出项目 → **确认发布版本（用当前 / 新建）** → 创建需求 → **经你确认后**启动工作流 → 可选进入产品构思。
...
1. 在需求页点击「网页创建」，填写所属项目、发布版本（默认当前版本）、标题、描述、验收标准和可选的目标仓库范围。
