...
钉钉组织的管理员可进入“用户管理”，点击“同步钉钉用户”批量导入通讯录成员。同步会递归读取钉钉部门中的用户并去重，但只保存用户资料和当前组织成员关系，不保存部门树或上下级关系。同步是增量操作：会新增或更新用户，不会自动删除、停用或降级 FlowX 中的现有成员。使用前需为钉钉应用开通通讯录部门和用户读取权限，并配置 `DINGTALK_APP_ID`、`DINGTALK_APP_SECRET`。
