...
云效自动化规则还可以调用 FlowX Webhook，将工作项事件定向发送给组织内的钉钉用户。接入前应先完成钉钉用户同步；FlowX 会依次按钉钉 `userid`、`unionId`、邮箱、账号和组织内唯一姓名匹配，成功发送的同一事件不会重复通知。管理员配置方式和云效请求体见仓库文档 `docs/yunxiao-webhook.md`。
