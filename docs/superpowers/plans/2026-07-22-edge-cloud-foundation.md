...
- [ ] 先写状态机测试：创建、认领、运行、完成、失败、取消。
- [ ] 实现 `createOrReuseSession`，同一幂等键返回同一会话。
- [ ] 实现活动会话冲突检测。
- [ ] 实现心跳和 `lastHeartbeatAt`。
- [ ] 实现终态保护和重复完成返回。
- [ ] 所有查询按现有组织/工作区权限过滤。
- [ ] Controller 只处理 DTO 和响应映射，状态逻辑留在 Service。
...
- [ ] 先写重复 `eventId/idempotencyKey` 测试。
- [ ] 校验协议版本、会话归属、事件类型和终态限制。
- [ ] 同一重复事件返回原事件，不重复写入。
- [ ] `execution.heartbeat` 更新会话心跳但保留事件历史。
- [ ] `execution.blocked` 只记录阻塞，不直接把工作流置为失败。
- [ ] 为事件列表提供稳定分页和时间排序。
