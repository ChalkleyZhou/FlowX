...
- `用例库`（`/quality/test-cases`）：通过高密度列表、搜索、优先级和用例库筛选维护大量 Workspace 共享用例及项目专属用例。列表操作区可查看、编辑或删除用例；编辑支持调整所属用例库、模块、用例编号、标题、优先级、前置条件、步骤、预期结果和标签，保存后用例版本自动递增，已冻结的提测快照不受影响。可下载 CSV 模板批量导入，每次最多 1000 条；模板中的步骤按单元格换行拆分，标签按英文逗号拆分，模块名称需已存在于目标用例库。文件会先完整校验，任何一行有误都不会写入。删除用例后，它不再进入新的提测范围，已经冻结的提测快照和历史执行结果仍会保留
