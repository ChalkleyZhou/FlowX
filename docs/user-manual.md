...
- `提测管理`（`/quality/test-requests`）：按项目和状态查看提测记录，选择项目版本下已完成的研发工作流发起提测草稿。提测前必须关联已确认且未过期的测试设计
...
- `执行记录`（`/quality/test-runs`）：查看初测、冒烟和 Bug 回归轮次及其状态
...
冒烟用例每次根据当前改动动态生成，只进入本次测试计划快照，不维护长期固定的冒烟库。提测前必须确认功能用例和冒烟结果；若没有适用场景，需要填写人工确认原因。
