...
FlowX 接收云效 Projex 自动化规则发送的原生“工作项数据”，通过云效项目成员 API 获取组织成员 `memberId`、云效 `userId` 和阿里云账号绑定 ID，按管理员确认的映射匹配 FlowX 用户，并发送个人工作通知。
...
配置完成后，组织管理员需要在 FlowX「设置」→「云效集成」中填写云效 `organizationIdentifier` 并启用云效通知。主管理员和子管理员均可维护该配置。停用不会删除配置和历史投递记录，重新启用即可恢复。未配置 Secret 或云效组织绑定时不能启用集成。
...
FlowX 使用工作项中的 `spaceIdentifier` 作为项目 ID，调用云效项目成员接口，并通过 `ReadMemberByUser` 和 `GetBindInfo` 补全组织成员 `memberId`、云效 `userId` 以及 `bindType=aliyunAccount` 的 `bindId`。Webhook 中通知对象的 `identifier` 对应阿里云账号绑定 ID，管理员在“设置”→“云效集成”加载成员后确认 FlowX 用户映射，三种身份都会保存；同一个云效用户跨项目可以复用映射。绑定信息暂时不可用时仍会展示成员，但对应字段为空。使用 AccessKey 时仍兼容通过 `dingTalkId` 匹配已同步的钉钉身份。不会使用姓名猜测接收人；同一 FlowX 用户同时属于多个角色时只发送一次，投递记录会保留其全部角色。
...
组织管理员可以在“未匹配人员”区域清空当前组织的未匹配记录。该操作不会影响成员映射、已匹配投递记录或 Webhook 原始数据。
