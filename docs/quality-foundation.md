...
一级导航 `测试与质量` 对应 `/quality`，展开三个独立子菜单：提测管理 `/quality/test-requests`、用例库 `/quality/test-cases`、执行记录 `/quality/test-runs`。用例库采用高密度列表和服务端分页，完整步骤与预期结果按需查看；支持下载 CSV 模板并批量导入，客户端先校验列结构和逐行内容，服务端再校验模块、编号重复和批量上限，任何错误都会中止整批写入。删除用例只会将定义归档并退出后续 AI 选例范围，不修改历史快照。提测创建只允许选择项目版本下已完成的研发工作流；AI 选例、覆盖检查和快照冻结仍由质量域 API 契约控制。
