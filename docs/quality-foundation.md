...
| `PATCH` | `/quality/test-case-modules/:id` | 重命名模块或调整上级模块；禁止模块循环和库内重名 |
| `DELETE` | `/quality/test-case-modules/:id` | 删除无子模块的模块；有效用例转为未分模块并递增用例版本 |
...
一级导航 `测试与质量` 对应 `/quality`，展开三个独立子菜单：提测管理 `/quality/test-requests`、用例库 `/quality/test-cases`、执行记录 `/quality/test-runs`。用例库采用高密度列表和服务端分页，完整步骤与预期结果按需查看；列表提供创建、查看、编辑和删除操作，编辑可调整所属用例库、模块、编号、标题、优先级、前置条件、步骤、预期结果和标签。列表工作区的“模块管理”支持按用例库新增、重命名、调整层级和删除模块；存在子模块时必须从叶子节点开始删除，删除模块只会将有效用例转为未分模块。每次保存用例编辑或因删除模块改变归属都会递增用例定义版本，已经冻结的提测快照仍保留选例时的内容和版本。用例库支持下载 CSV 模板并批量导入，客户端先校验列结构和逐行内容，服务端再校验模块、编号重复和批量上限，任何错误都会中止整批写入。删除用例只会将定义归档并退出后续 AI 选例范围，不修改历史快照。提测创建只允许选择项目版本下已完成的研发工作流；AI 选例、覆盖检查和快照冻结仍由质量域 API 契约控制。
