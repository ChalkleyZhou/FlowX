...
- 支持通过固定地址 `/api/yunxiao-webhooks` 接收云效自动化规则的原生工作项数据，并根据负责人匹配 FlowX 用户后发送钉钉个人通知。云效 Secret 通过 `X-Projex-Signature` 校验，服务端配置 `YUNXIAO_WEBHOOK_SECRET`，不使用个人 API Token。详见[云效 Webhook 钉钉通知接入](docs/yunxiao-webhook.md)。
