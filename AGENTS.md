...
- 后端：`apps/api`，NestJS + TypeScript + Prisma + SQLite，负责认证、工作区/仓库、需求、工作流、AI 执行器、审查沉淀和部署集成。
- 前端：`apps/web`，React + Vite + Tailwind + Radix/shadcn 风格组件，负责管理控制台。
...
- 修改工作流编排、ideation 编排、状态机、Prisma schema、认证、凭据、AI executor、API 边界时，把它们视为高风险区域：先补或更新测试，再改实现。
