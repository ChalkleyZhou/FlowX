...
- 后端：`apps/api`，NestJS + TypeScript + Prisma + SQLite，负责认证、工作区/仓库、需求、工作流、AI 执行器、审查沉淀、排期、项目简报和部署集成。
- 前端：`apps/web`，React + Vite + Tailwind + Radix/shadcn 风格组件，负责管理控制台、工作流详情、项目简报、投递目标和排期视图。
...
- `apps/api/src/briefings`：项目简报、代码事件聚合、AI 总结、定时生成和投递目标。
...
- 修改工作流编排、ideation 编排、状态机、Prisma schema、认证、凭据、AI executor、项目简报/投递、排期或 API 边界时，把它们视为高风险区域：先补或更新测试，再改实现。
...
- 变更 `apps/api/src/briefings` 或 `apps/api/src/schedule` 前，优先添加或更新相关 API 测试。
